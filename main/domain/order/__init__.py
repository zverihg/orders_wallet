...
def get_orders_by_customer(customer_id: UUID, *, limit: int = 50, offset: int = 0):
    return _query_service.get_orders_by_customer(customer_id=customer_id, limit=limit, offset=offset)


def capture_payment(order_id: UUID, idempotency_key: str | None = None):
    return _payment_service.capture_payment(order_id=order_id, idempotency_key=idempotency_key)
